...
class OccupancyLoss(BaseLoss):
    ...
    def _get_prediction_weights(self, num_predictions):
        if num_predictions <= 0:
            raise ValueError("pred_occ must contain at least one prediction")
        # Evaluation keeps only final D3.  Use unit weight rather than trying
        # to match the three training deep-supervision weights.
        ...
        if len(self.prediction_weights) != num_predictions:
            raise ValueError(
                "prediction_weights must match the selected pred_occ: "
    ...
    def loss_voxel(self, pred_occ, sampled_xyz, sampled_label, occ_mask=None):
        ...
        tot_loss = 0.
        prediction_weights = self._get_prediction_weights(len(pred_occ))
        ...
        for prediction_index, semantics in enumerate(pred_occ):
            if occ_mask is not None:
