    ...
    if data_type == 'kitti':
        miou_metric = MeanIoU(
            ...
            ['car', 'bicycle', 'motorcycle', 'truck', 'other-vehicle',
             ...
             'fence', 'vegetation', 'terrain', 'pole', 'traffic-sign', 'other-structure', 'other-object'],
        ...
        with torch.no_grad():
            for i_iter_val, data in enumerate(val_dataset_loader):
                ...
                if i_iter_val % print_freq == 0 and local_rank == 0:
                    ...
                    detailed_loss = ', '.join(detailed_loss)
                    logger.info(detailed_loss)
                del result_dict, loss_input, loss, loss_dict

        miou, iou2 = miou_metric._after_epoch()
        logger.info(f'mIoU: {miou}, iou2: {iou2}')
        if best_mIoU < miou:
            logger.info(f'New Best mIoU! ({best_mIoU} -> {miou}), and best epoch is {epoch}')
            best_mIoU = miou
            best_epoch = epoch
        else:
            logger.info(f'Best mIoU! ({best_mIoU}), and best epoch is {best_epoch}')
