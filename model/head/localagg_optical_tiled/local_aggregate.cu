	...
	const int H, int W, int D)
{
	...
	auto byte_opts = means3D.options().dtype(torch::kUInt8);
	...
	// Allocate all temporary buffers on the input tensor's CUDA device. This is
	// required when distributed training uses a non-default local GPU.
	...
	return std::make_tuple(rendered, out_logits, out_bin_logits, out_density, geomBuffer, binningBuffer, imgBuffer);
}
